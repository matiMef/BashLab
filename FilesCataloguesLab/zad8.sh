...
for folder in aaa bbb; do
    # Tworzymy odpowiadający katalog w zapasach
    mkdir -p "ddd/zapasy/$dir"

    # Przechodzimy przez wszystkie pliki i katalogi w danym katalogu
    for plik in "$folder"/**/*; do
        # Sprawdzamy, czy to plik i czy NIE mamy do niego prawa zapisu
        if [[ -f "$plik" && ! -w "$plik" ]]; then
            #Dodawanie brakujących katalogów
            mkdir -p "ddd/zapasy/$(dirname "$plik")"
            cp "$plik" "ddd/zapasy/$plik"
        fi
    done
done

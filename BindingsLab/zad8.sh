...
for plik in dane/pierwiastki/*; do
    sprawdzenie=0  # Resetowanie sprawdzenia na początku pętli

    if [[ ! -L "$plik" ]]; then
	    for plik2 in dane/elements/*; do
		    if [ "$(basename "$plik")" == "$(basename "$plik2")" ]; then
              		sprawdzenie=1
                	break
            	    fi
             done
...
for plik in dane/elements/*; do
    ...
    if [ ! -L "$plik" ]; then
        for plik2 in dane/pierwiastki/*; do
            if [ "$(basename "$plik")" == "$(basename "$plik2")" ]; then
                sprawdzenie=1
                break
            fi
